...
const Register = () => {
  ...
  const register = async (formData) => {
    ...
    try {
      ...
      if (e.response && e.response.status === 400) {
        ...
        enqueueSnackbar(
          "Something went wrong, check thatthe backend is running, reachable and returns valid JSON.",
          { variant: "error"}
        )
      }
    }
  ...
  const validateInput = (data) => {
    ...
    if (!data.username) {
      enqueueSnackbar("Usernmae is a required field", { variant: "warning" });
      return false;
    }
    if (data.username.length < 6) {
      enqueueSnackbar("Usernmae atleast should be 6 characters", {
        varient: "warning",
      });
      return false;
    }
    ...
    if (data.password.length < 6 ) {
      enqueueSnackbar("Psssword must me atleat 6 characters", {
        varient: "warning",
      });
      return false;
    }
